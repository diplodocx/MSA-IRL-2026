#!/usr/bin/env python3
"""
Прототип многоагентной дорожной среды — агентная версия с логированием (log_mas_simulation).
...
log_rows = []   # список dict'ов с данными
...
def broadcast():
    ...
    with log_lock:
        state['logging_active'] = log_cfg['active']
...
def save_log():
    """Сохраняет накопленные строки в CSV-файл в папке ./expert/."""
    os.makedirs('./expert', exist_ok=True)
    filename = 'expert_' + datetime.now().strftime('%Y%m%d%H%M%S') + '.csv'
    filepath = os.path.join('./expert', filename)
    with log_lock:
        rows = list(log_rows)
...
def on_start_logging():
    ...
    with log_lock:
        ...
        log_cfg['active'] = True
        log_rows.clear()
    _log_thread = threading.Thread(target=log_loop, daemon=True)
    _log_thread.start()
    broadcast()
...
def on_stop_logging():
    with log_lock:
        if not log_cfg['active']:
            return
        log_cfg['active'] = False

    filename = save_log()
    broadcast()
    socketio.emit('log_saved', {'filename': filename})
...
<title>MAS — агентная среда (с логированием)</title>
...
.log-badge.active   { color: var(--log-on); background: #a259ff18; }
.log-badge.inactive { color: var(--dim);    background: transparent; }
...
/* Logging buttons */
#btn-start-log { background: var(--log-on); color: #fff; }
#btn-stop-log  { background: var(--log-off); color: var(--text); }
#btn-stop-log.armed { background: #7c3aed; color: #fff; }
...
#log-notify.show { opacity: 1; }
</style>
</head>
<body>

<h1>MAS — многоагентная дорожная среда (с логированием)</h1>
...
<div class="counters">
  ...
  <div>Лог: <span id="log-badge" class="log-badge inactive">ВЫКЛ</span></div>
</div>
...
<!-- Уведомление о сохранении лога -->
<div id="log-notify"></div>
...
sock.on('state', s => {
  ...
  // Статус логирования
  const logActive = s.logging_active;
  const logBadge = id('log-badge');
  logBadge.textContent = logActive ? 'ЗАПИСЬ' : 'ВЫКЛ';
  logBadge.className   = 'log-badge ' + (logActive ? 'active' : 'inactive');
  id('btn-stop-log').className = logActive ? 'armed' : '';
});

sock.on('log_saved', data => {
  showNotify('Лог сохранён: ' + data.filename);
});

function showNotify(msg) {
  const el = id('log-notify');
  el.textContent = msg;
  el.classList.add('show');
  setTimeout(() => el.classList.remove('show'), 4000);
}

function startLogging() { sock.emit('start_logging'); }
function stopLogging()  { sock.emit('stop_logging'); }
...
if __name__ == '__main__':
    print('Сервер запущен: http://localhost:5004')
