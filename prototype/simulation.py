#!/usr/bin/env python3
"""
Прототип многоагентной дорожной среды.
...
Каждые main_delay секунд машина добавляется в конец main_line.
Каждые transfer_delay секунд машина перемещается из начала main_line
в конец выбранной (line_choose) трассы highway1 или highway2.
Каждая highway имеет свою скорость выезда (drain_delay_h1 / drain_delay_h2).
"""
...
def padded(q, capacity):
    """Новейшая машина — крайняя СЛЕВА, старейшая — крайняя справа из заполненных.
    Пустые ячейки — справа. Так машины появляются у стыка (левый край highways
    или правый край main_line) и движутся к выходу.
    """
    lst = list(q)[::-1]          # index 0 = newest
    return lst + [0] * (capacity - len(lst))
...
<title>Дорожная среда — прототип</title>
...
/* colour accents for per-highway sliders */
...
<h1>Многоагентная дорожная среда</h1>

<!-- Road scene: main_line LEFT, highways RIGHT -->
<div class="scene">
...
<div class="controls">
  ...
  <div class="ctrl-group">
    ...
    <input type="range" min="0.2" max="4" step="0.1" value="1.0"
  ...
  <div class="ctrl-group">
    ...
    <input type="range" min="0.1" max="4" step="0.1" value="0.7"
  ...
  <div class="ctrl-group">
    ...
    <input class="accent-h1" type="range" min="0.1" max="8" step="0.1" value="2.0"
  ...
  <div class="ctrl-group">
    ...
    <input class="accent-h2" type="range" min="0.1" max="8" step="0.1" value="2.0"
